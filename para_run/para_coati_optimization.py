"""
使用 Coati 方法（长鼻浣熊优化算法）对晶体结构进行批量优化的脚本。
...
def optimize_structure(input_file, output_file, optimizer, coati_params, entropy_params):
    """
    使用Coati算法优化单个结构文件并保存结果。

    Args:
        input_file (str): 输入CIF文件路径。
        output_file (str): 输出CIF文件路径。
        optimizer (StructOptimizer): 结构优化器实例。
        coati_params (dict): Coati算法参数字典。
    ...
    try:
        ...
        final_structure = optimizer.relax_coati(
            initial_structure,
            n_coatis=coati_params['n_coatis'],
            max_iter=coati_params['max_iter'],
...
def process_input_path(input_path, output_dir, optimizer, coati_params, entropy_params):
    """
    处理输入路径（可以是单个文件或目录）。
    """
    ...
    if os.path.isdir(input_path):
        ...
        for cif_file in cif_files:
            ...
            result = optimize_structure(cif_file, output_file, optimizer, coati_params, entropy_params)
            if result:
        ...
        output_file = os.path.join(output_dir, f"{base_name}_optimized.cif")
        optimize_structure(input_path, output_file, optimizer, coati_params, entropy_params)
    else:
...
def main():
    """主函数，执行Coati结构优化流程。"""
    parser = argparse.ArgumentParser(description='使用Coati方法（长鼻浣熊优化算法）优化晶体结构')
    parser.add_argument('--input', '-i', type=str, default='../test_data', help='输入CIF文件或包含CIF文件的目录路径')
    parser.add_argument('--output', '-o', type=str, default='../test_data/coati_optimizer', help='输出目录，用于保存优化后的结构')

    # Coati相关参数
    parser.add_argument('--coatis', type=int, default=20, help='Coati算法的长鼻浣熊数量 (默认: 20)')
    parser.add_argument('--iterations', '-n', type=int, default=50, help='Coati算法的最大迭代次数 (默认: 50)')
    ...
    print(f"Coati参数: 种群数量={coati_params['n_coatis']}, 迭代次数={coati_params['max_iter']}")
    if entropy_params['use_entropy']:
    ...
    process_input_path(args.input, args.output, optimizer, coati_params, entropy_params)
    print("\n所有优化任务完成！")
